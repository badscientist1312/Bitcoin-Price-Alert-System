...
while True:
    ...
    try:
         if market_price < sell_price:
             ...
             response1=sms.send_sms("The Bitcoins are at price ={} You can Invest now if you want".format(market_price))
             print("Status of Sms at Twilo:"+str(response1.status))

         elif market_price > sell_price:
             bolt.digitalWrite("1","HIGH")#BUZZER gets "ON
             response1=sms.send_sms("The Bitcoins are at price ={} You need to be cautious".format(market_price))
             print("Status of Sms at Twilo:"+str(response1.status))
